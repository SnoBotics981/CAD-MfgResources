 ...
 *  PFS: 02/12/2016 (search for PFS to find the changes listed below) from http://www.thingiverse.com/thing:193647
 *		1) removed deprecated 'assign'
 *		2) replaced deprecated 'triangles' with 'faces'
 *		3) made width of wingnut configurable in module 'wing' (had problems with origianl 3mm thick wing breaking)
 *
...
//PFS: added configurable wing_thickness (for strength), see cylinder in module wing
wing_thickness = 3;
...
if (type=="nut" && nut_type=="wingnut")
{
	...
	translate([(nut_diameter/2)+wing_radius-1,wing_thickness/2,wing_radius/2+1])rotate([90,0,0])wing(); //attach wing, PFS addes wing_thickness
	mirror(1,0,0)translate([(nut_diameter/2)+wing_radius-1,wing_thickness/2,wing_radius/2+1])rotate([90,0,0])wing(); //attach wing, PFS added wing_thickness
}
...
module wing()
{
	difference()
	{
		cylinder(r=wing_radius,h=wing_thickness,$fn=64);
		union()  
		{
...
module full_thread(ttn,st,sn,zt,lfxy,or,ir)
{
  if(ir >= 0.2)
  {
    ...
    {
        for(j=[0:sn-1])
		{ //PFS: removed deprcated assign function
			pt = [	[0,                  0,                  i*st-st            ],
			...
			//PFS: replaced deprecated 'triangles' with 'faces'
